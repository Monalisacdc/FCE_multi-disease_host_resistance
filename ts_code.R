...
######
#             Selecting the specific time to zadoks 
...
# ============================================================
# 2. Modelo beta para severidade final
# ============================================================
...
tssev = p_final_class
(tssev|lrsev)/
(pmsev+ plot_spacer()) + plot_annotation(tag_levels = "A")
ggsave("plot2_sev.png", width =10, height = 10, bg = "white", dpi = 1000)
...
# Juntar classes às médias ajustadas
...
# ---------------------------
# 8) Agreement: breeder class vs functional clusters
# ---------------------------
...
# ---------------------------
# 9) Functional separation (R2-like) + PCoA
# ---------------------------
...
# ---------------------------
# 10) Choose k (optional): silhouette + functional R2 + ARI to breeder class
# --------------------------

eval_k_fast <- function(m1, ks = c(3,4), geno_class,
                        hc_method = "ward.D2") {
  
  # 1) matriz de distância (uma vez)
  ...
  # Se algum genótipo não tiver classe, restringe D
  ...
  purrr::map_dfr(ks, function(k) {
    ...
    # Silhouette: NA se algum cluster tiver só 1 genótipo
    if (any(table(cl) < 2)) {
      mean_sil <- NA_real_
    } else {
    ...
    # ARI com classes baseadas em AACPD
    ari_aacpd <- mclust::adjustedRandIndex(
      as.factor(cl),
      as.factor(comp$aacpd_cl))
    
    # ARI com classes baseadas em severidade final
...
# ---------------------------
# 13) Features from fitted curves: t5, vmax (and tests)
# ---------------------------
# Compute genotype features from fitted mean curves (t5, vmax, t_vmax)
  curve_features <- function(pred_df, threshold = 0.05) {
    ...
    pred_df %>%
      ...
      group_modify(~{
        ...
        # Tempo para atingir 5% de severidade
        t5 <- NA_real_
  ...
  # Por classe baseada em severidade final
