...
dat_all %>%
  group_by(Env) %>%
  summarise(n_genotipos = n_distinct(geno), .groups = "drop") %>%
  arrange(Env)
```
...
# filter(!is.na(time)) %>%
 # group_by(geno, env, powdery_mildew) %>%
...
final_sev <- filtered_data %>%
  ...
  summarise(final_sev_pm = max(powdery_mildew, na.rm = TRUE), .groups = "drop")

env_valid <- final_sev %>%
  group_by(env) %>%
  summarise(max_final_pm = max(final_sev_pm, na.rm = TRUE), .groups = "drop") %>%
  filter(max_final_pm > 10)
...
resistencia_tbl <- tribble(
  ...
  mutate(
    ...
    aacpd_cl = factor(aacpd_cl, levels = c("R", "MR", "MS", "HS"), ordered = TRUE),
...
pmsev = p_final_class2
(p_final_class2|p_dend_final2)+plot_annotation(tag_levels = "A")
# ggsave("sev_pm.png", width = 13, height = 11, bg = "white", dpi = 1000)
...
zadoks_range_geno_env <- dat_ready %>%
  dplyr::filter(
    !is.na(env),
    !is.na(geno),
    !is.na(time)
  ...
  dplyr::group_by(env, geno) %>%
  dplyr::summarise(
    n_eval = dplyr::n_distinct(time),
    zadoks_start = min(time, na.rm = TRUE),
    zadoks_end = max(time, na.rm = TRUE),
    interval = zadoks_end - zadoks_start,
    zadoks_values = paste(sort(unique(time)), collapse = ", "),
    .groups = "drop"
  ) %>%
  dplyr::arrange(geno, env)

zadoks_range_geno_env
...
aacpd_dat <- dat_ready %>%
  ...
  dplyr::mutate(
    aacpd_rel = aacpd / (time_max - time_min),
    env = as.factor(env),
    geno = as.factor(geno)
  )
...
aacpd_dat <- aacpd_dat %>%
  mutate(log_aacpd = log(aacpd_rel + 1))
...
groups_aacpd <- cutree(hc_aacpd, k = 4)
...
emm_aacpd_df <- emm_aacpd_df %>%
  ...
  dplyr::mutate(
    geno = factor(geno_chr, levels = geno_chr),
    aacpd_class = factor(aacpd_class, levels = c("R", "MR", "MS", "HS")))
...
# ggsave("aacpd_sev_pm.png", width = 13, height = 11, bg = "white", dpi = 1000)
...
```{r}
dat_ready %>%
  dplyr::filter(geno == "TBIO Audaz") %>%
  dplyr::mutate(y_orig = tan_spot / 100) %>%
  ggplot(aes(x = time, y = y_orig, group = env)) +
  geom_line(alpha = 0.5) +
  geom_point(size = 1.8, alpha = 0.7) +
  theme_bw() 
 
audaz_mean_curve <- dat_ready %>%
  mutate(
    y_orig = tan_spot / 100
  ) %>%
  filter(geno == "TBIO Audaz") %>%
  group_by(time) %>%
  summarise(
    mean_sev = mean(y_orig, na.rm = TRUE),
    sd_sev = sd(y_orig, na.rm = TRUE),
    n_env = n_distinct(env),
    se_sev = sd_sev / sqrt(n_env),
    lower = mean_sev - se_sev,
    upper = mean_sev + se_sev,
    .groups = "drop")
...
dat_ready %>%
  ...
  filter(geno == "TBIO Audaz") %>%
  ggplot(aes(x = time, y = y_orig * 100, group = env)) +
  geom_line(alpha = 0.35) +
  geom_point(size = 1.5, alpha = 0.5) +
  geom_line(
    data = audaz_mean_curve,
    aes(x = time, y = mean_sev * 100, group = 1),
    inherit.aes = FALSE,
    linewidth = 1.3) +
  theme_bw()
```
...
length(unique(dat_ready$time))
range(dat_ready$y, na.rm=TRUE)
sum(dat_ready$y %in% c(0,1), na.rm=TRUE)
...
m1$pred %>%
  names()
diag = diagnose_curves(m1, grid_n = 200)
diag$plots

library(patchwork)

wrap_plots(diag$plots[1:4], ncol = 2)
m1$data
summary(m1$gam)
...
#write_xlsx(env_por_geno, "pm_env.xlsx")

dat_ready |> filter(Genotype == "BRS Sanhaco")
```
...
head(curves_env)
#write_csv(curves_env, "curves_pm_env.csv")
getwd()
```
...
class_for_D <- class_for_D %>%
  ...
  grp_aacpd <- droplevels(factor(class_for_D$aacpd_cl, levels = c("R","MR", "MS", "HS")))
  grp_severity <- droplevels(factor(class_for_D$severity_cl, levels = c("R","MR", "MS", "HS")))
  grp_functional <- droplevels(factor(class_for_D$functional_cl, levels = c("R","MR", "MS", "HS")))
...
ari_two_points <- function(z1, z2, df, k = 4) {
...
# ggsave("pm_plot.png", width = 8, height = 13, bg = "white", dpi = 1000)
```
